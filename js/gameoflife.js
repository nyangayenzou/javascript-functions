...
const getNeighborsOf = ([x, y]) => {
  [x-1,y+1],[x,y+1],[x+1,y+1],[x-1,y],[x+1,x],[x-1,y-1],[x,y-1],[x+1,y-1]
};

const getLivingNeighbors = (cell, state) => {
  return getNeighborsOf(cell);filter((n)=>.bind(state)(n))
};

const willBeAlive = (cell, state) => {
  const livingNeighbors = getLivingNeighbors(cell,state)
  return(livingNeighbors.length === 3 || (contains.call(
    state,cell
